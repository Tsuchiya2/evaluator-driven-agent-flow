...
**Examples**:
- TypeScript with `strict: true` + 90% coverage = 4.8/10.0
- Python with mypy `--strict` + 70% coverage = 4.0/10.0
- JavaScript (no types) = 2.0/10.0 (baseline)
...
**Examples**:
- 0 errors, 0 warnings = 5.0/10.0
- 2 errors, 5 warnings in 10 files = 4.65/10.0
- 10 errors, 20 warnings in 5 files = 2.8/10.0
...
**Examples**:
- Average: 5, Max: 8, 0 over threshold = 5.0/10.0
- Average: 12, Max: 20, 20% over threshold = 3.2/10.0
- Average: 8, Max: 35, 10% over threshold = 3.8/10.0
...
// Need to normalize ALL to 0-10 scale
```
...
async function runCodeReviewGate(prNumber: number): Promise<GateResult> {
  ...
  if (qualityReport.passFail === 'FAIL') {
    return {
      status: 'BLOCKED',
      reason: `Code quality score ${qualityReport.scores.overall}/10.0 is below threshold ${qualityReport.threshold}`,
...
```json
{
  ...
  "result": {
    "status": "PASS",
    "threshold": 3.5,
    "message": "Code quality meets standards (4.2/10.0 ≥ 3.5)"
  }
}
```
...
✅ **Normalized Scoring** - All languages scored on same 0-10 scale
